...
var zcaudioCbName;
(function (zcaudioCbName) {
    zcaudioCbName["onWaiting"] = "waiting";
    zcaudioCbName["onError"] = "error";
    zcaudioCbName["onTimeUpdate"] = "playing";
    zcaudioCbName["onCanplay"] = "canPlay";
    zcaudioCbName["onPause"] = "pause";
    zcaudioCbName["onEnded"] = "ended";
    zcaudioCbName["setAudio"] = "setAudio";
    zcaudioCbName["updateAudio"] = "updateAudio";
    zcaudioCbName["seek"] = "seek";
	zcaudioCbName["playbackRate"] = "playbackRate";
    zcaudioCbName["onStop"] = "stop";
    zcaudioCbName["syncStateOn"] = "syncStateOn";
})(zcaudioCbName || (zcaudioCbName = {}));
let zcaudioCbNameArr = [];
for (const key in zcaudioCbName) {
    if (Object.prototype.hasOwnProperty.call(zcaudioCbName, key)) {
        const item = zcaudioCbName[key];
        zcaudioCbNameArr.push(item);
    }
}
 ...
 * @method setRender(data)             指定音频, 渲染到zcaudio组件
 ...
 *
 * **/
class zcAudio extends util_1.EventBus {
    constructor(options) {
        super();
        this.loading = false;
        this.renderIndex = 0;
        ...
        this.throttlePlaying = util_1.throttle(() => {
            this.emit(zcaudioCbName.onTimeUpdate, this.playinfo);
            this.syncStateEmit();
        }, 1000);
    ...
    checkEventParams(event, action, fn) {
        if (zcaudioCbNameArr.indexOf(event) < 0) {
            console.error(`参数${event}错误, 必须为${zcaudioCbNameArr.join(" | ")}中某一项`);
            return false;
        }
     ...
     * @param {<zcaudioCbName>}   event     回调名称枚举值
     ...
     * @param {<zcaudioCbName>}        event      回调名称枚举值,具体看zcaudioCbName
    ...
    onWaitingHandler() {
        ...
        this.emit(zcaudioCbName.onWaiting, true);
        this.syncStateEmit();
    }
    onCanplayHandler() {
        ...
        this.emit(zcaudioCbName.onCanplay, this.playinfo);
    ...
    onPauseHandler() {
        ...
        this.emit(zcaudioCbName.onPause);
        this.syncStateEmit();
    }
    onStopHandler() {
        this.commit("setPause", true);
        this.emit(zcaudioCbName.onStop);
        this.syncStateEmit();
    }
    onEndedHandler() {
        this.commit("setPause", true);
        ...
        this.emit(zcaudioCbName.onEnded);
        this.syncStateEmit();
        //续播
    ...
    onErrorHandler() {
        ...
        this.emit(zcaudioCbName.onError);
        this.syncStateEmit();
        if (this.continuePlay) {
            this.changeplay(1);
        }
    ...
    /**
     * @description 注册一个实时获取zcAudio属性的方法
     * @param {String}        action      自定义业务名
     * @param {Funtion}     fn        实时获取zcAudio属性回调
     * @returns undefined
     * **/
    syncStateOn(action, fn) {
        typeof fn === "function" && this.on(zcaudioCbName.syncStateOn, action, fn);
    }
    /**
     * @description 卸载实时获取zcAudio属性的方法
     * @param {String}        action      自定义业务名
     * @returns undefined
     * **/
    syncStateOff(action) {
        this.off(zcaudioCbName.syncStateOn, action);
    }
    /**
     * @description 订阅实时获取zcAudio属性的方法
     * @returns undefined
     * **/
    syncStateEmit() {
        this.emit(zcaudioCbName.syncStateOn, {
    ...
    seek(value) {
        ...
        // setTimeout(() => {
        //   this.emit(zcaudioCbName.seek, this.playinfo.current);
        // }, 0);
        this.emit(zcaudioCbName.seek, this.playinfo.current);
    }
	...
	/**
	 * @description 快进
	 * @param {Number}        value      倍速值
	 * @returns undefined
	 * **/
	playbackRate(value) {
		this.audioCtx.playbackRate=value;
	}
    ...
    operation() {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const { src: renderSrc, title: renderTitle, singer: renderSinger, coverImgUrl: renderCoverImgUrl, chapter_id:chapter_id,type:type} = this.renderinfo;
    ...
    setAudio(data) {
        this.audiolist = [...data];
        this.emit(zcaudioCbName.setAudio, this.audiolist);
        this.syncStateEmit();
    }
    /**
     * @description 添加音频
    ...
    updateAudio(data) {
        ...
        this.emit(zcaudioCbName.updateAudio, this.audiolist);
        this.syncStateEmit();
    }
    /**
    ...
    setRender(data) {
        ...
        if (typeof data === "number" || typeof data === "string") {
            ...
            this.renderinfo = {
                ...
                current: "00:00",
                duration: this.audiolist[this.renderIndex].duration||"00:00",
                current_value: 0,
				audition_time:parseInt(this.audiolist[this.renderIndex].audition_time)||300,
                duration_value: 100,
				chapter_id:this.audiolist[this.renderIndex].chapter_id||0,
				type:this.audiolist[this.renderIndex].type||0
            };
        }
        else {
		...
		// 获取时长，需要优化
		// this.audioCtx.src=this.renderinfo.src;
		// this.audioCtx.pause();
		// this.commit("setPlayinfo",this.renderinfo);
		this.syncStateEmit();
    }
    //当前索引
    get playIndex() {
...
exports.default = zcAudio;
zcAudio.version = "1.0.0";
